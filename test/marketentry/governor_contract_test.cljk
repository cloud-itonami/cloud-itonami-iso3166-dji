  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems DJ"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems DJ" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest rccm-entity-missing-is-held-and-unoverridable
  (testing "missing RCCM-registered Djiboutian entity -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
...
(deftest rccm-registration-unverified-is-held-and-unoverridable
  (testing "unverified RCCM registration record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:rccm-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
