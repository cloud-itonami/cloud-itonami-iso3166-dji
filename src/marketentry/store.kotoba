(ns marketentry.store
  "SSoT for the DJI market-entry compliance actor, behind a `Store`
  ...
  every prior cloud-itonami actor in this fleet uses.
    ...
    - `DatomicStore` -- backed by `langchain.db`, a Datomic-API-compatible
                        EAV store. Built directly on `langchain-store.core`
                        (`ls/enc`/`ls/dec*`/`ls/read-stream`/
                        `ls/append-blob!`) rather than a hand-rolled
                        EDN-blob codec -- see ADR-2607141600 (~190
                        cloud-itonami actors hand-roll the identical
                        `enc`/`dec*` two-liner; new stores must not add
                        to that count).
...
(defn demo-data
  ...
  eng-1 clean, eng-2 unknown-jurisdiction (no spec-basis), eng-3
  fee-mismatch, eng-4 missing-RCCM-entity, eng-5 missing-RCCM-
  registration-verification, eng-6 a second clean case with a smaller
  fee -- mirroring cloud-itonami-iso3166-ago's 6-engagement demo
  shape."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems DJ" :portal "BOAMP (marchespublics.gouv.dj)"
             ...
             :requires-rccm-registration? true :rccm-registration-verified? true
             :drafted? false :submitted? false
             :jurisdiction "DJI" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "BOAMP (marchespublics.gouv.dj)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-rccm-entity? true :has-rccm-entity? true
             :requires-rccm-registration? true :rccm-registration-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "BOAMP (marchespublics.gouv.dj)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "DJI" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "BOAMP (marchespublics.gouv.dj)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "DJI" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "BOAMP (marchespublics.gouv.dj)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-rccm-entity? true :has-rccm-entity? true
             :requires-rccm-registration? true :rccm-registration-verified? false
             :drafted? false :submitted? false
             :jurisdiction "DJI" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "BOAMP (marchespublics.gouv.dj)"
...
(def ^:private schema
  (ls/identity-schema
   [:engagement/id
    :assessment/engagement-id
    :ledger/seq
    :draft-record/seq
    :submit-record/seq
    :draft-sequence/jurisdiction
    :submit-sequence/jurisdiction]))
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               ...
                               requires-rccm-registration? rccm-registration-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-rccm-registration?)   (assoc :engagement/requires-rccm-registration? requires-rccm-registration?)
    (some? rccm-registration-verified?)   (assoc :engagement/rccm-registration-verified? rccm-registration-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-rccm-registration? :engagement/rccm-registration-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-rccm-registration? (boolean (:engagement/requires-rccm-registration? m))
     :rccm-registration-verified? (boolean (:engagement/rccm-registration-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-dji"})
