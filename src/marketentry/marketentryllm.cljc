(ns marketentry.marketentryllm
  ...
  the DJI public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real CNMP/BOAMP portal
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world CNMP/BOAMP portal
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
     ...
     :rationale  (if e
                   (str "has-rccm-entity?=" (:has-rccm-entity? e)
                        " rccm-registration-verified?=" (:rccm-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-rccm-entity? e))
                              (:has-rccm-entity? e))
                          (or (not (:requires-rccm-registration? e))
                              (:rccm-registration-verified? e)))
                   0.9 0.3)}))
